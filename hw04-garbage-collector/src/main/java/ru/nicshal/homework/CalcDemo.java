...
/*
-Xms256m - начальный размер (в байтах) кучи, которую JVM выделяет при старте приложения.
-Xmx256m - задает максимальный размер (также в байтах) кучи. Если приложение пытается использовать больше памяти, чем задано параметром -Xmx, то JVM может выбросить исключение OutOfMemoryError
