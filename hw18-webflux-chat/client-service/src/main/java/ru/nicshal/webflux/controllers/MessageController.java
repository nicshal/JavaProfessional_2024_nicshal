...
public class MessageController {
    ...
    private static final long KING_ROOM_ID = 1408;
    ...
    @SendTo({TOPIC_TEMPLATE + "{roomId}", TOPIC_TEMPLATE + KING_ROOM_ID})
    public Message getMessage(@DestinationVariable("roomId") String roomId, Message message) {
        logger.info("get message:{}, roomId:{}", message, roomId);
        Message newMessage = null;
        if (roomId.equals(String.valueOf(KING_ROOM_ID))) {
            logger.info("Can not send message from room {}", KING_ROOM_ID);
        } else {
            saveMessage(roomId, message)
                    .subscribe(msgId -> logger.info("message send id:{}", msgId));
            newMessage = new Message(HtmlUtils.htmlEscape(message.messageStr()));
        }
        return newMessage;
    }
    ...
    public void handleSessionSubscribeEvent(SessionSubscribeEvent event) {
        ...
        Flux<Message> fluxMessage = roomId == KING_ROOM_ID ? getAllMessages() : getMessagesByRoomId(roomId);
        fluxMessage
